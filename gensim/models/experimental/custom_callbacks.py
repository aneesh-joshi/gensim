...
class MPValidationCallback(Callback):
    """Callback for providing validation metrics on the model trained so far"""
    def __init__(self, test_data):
        """
        Parameters
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        y = self.test_data["y"]
        doc_lengths = self.test_data["doc_lengths"]
        query_lens = self.test_data["query_len"]
        doc_lens = self.test_data["doc_len"]

        predictions = self.model.predict(x={"query": X1, "doc": X2, 'query_len':query_lens, 'doc_len': doc_lens})

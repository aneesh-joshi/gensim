...
from custom_layers import BiLSTM
from custom_layers import MultiPerspectiveMatch
from custom_layers import SequenceMask
...
class BiMPM(utils.SaveLoad):
    ...
    def train(self, queries, docs, labels, word_embedding=None,
              ...
              validation_data=None, topk=20, target_mode='ranking', verbose=1, batch_size=100, steps_per_epoch=100):
    ...
    def predict(self, queries, docs, silent=True):
        ...
        predictions = self.model.predict(x={'query': indexed_long_query_list, 'doc': indexed_long_doc_list, 
            'query_len': np.array(long_query_len), 'doc_len': np.array(long_doc_len)})
    ...
    def save(self, fname, *args, **kwargs):
        ...
        super(BiMPM, self).save(*args, **kwargs)
    ...
    def load(cls, *args, **kwargs):
        ...
        """
        fname = args[0]
        gensim_model = super(BiMPM, cls).load(*args, **kwargs)
        keras_model = load_model(
                            fname + '.keras', custom_objects={'rank_hinge_loss': rank_hinge_loss, 'BiLSTM': BiLSTM,
                                                'MultiPerspectiveMatch': MultiPerspectiveMatch, 'SequenceMask': SequenceMask}
                        )
    ...
    def _get_keras_model(self, embed_trainable=False):
        """Builds and returns the keras class for bimpm model
        ...
        flat_drop = Dropout(rate=dropout_rate)(flat)
